...
private:
  ...
  {
    float fx, fy, cx, cy, x_3d, y_3d, z_3d, d;
    ...
    for (int row = 0; row < input.rows; ++row) {
      for (int col = 0; col < input.cols; ++col) {
          ...
          x_3d = ((float)row - cx) * d / fx;
          y_3d = ((float)col - cy) * d / fy;
          z_3d = d;
  ...
  {
    if(publisher_->get_subscription_count() == 0 || !intrinsic_obtained){return;}
    ...
    out_pointcloud.header = msg->header;
